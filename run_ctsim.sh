...
$COMMAND run --rm -it --hostname=ctsim-container -v $DATA_PATH:/data -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$X11_DISPLAY --security-opt=label=type:container_runtime_t -p 10001:10001 $IDMAP_ARGS $USER_ARGS docker.io/tsandmann/ctsim:$VERSION
$XHOST_CMD -si:localuser:$USER
