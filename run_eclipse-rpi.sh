...
#$COMMAND run --rm -it --hostname=ctbot-container -v $DATA_PATH:/data -v /tmp/.X11-unix:/tmp/.X11-unix -e DISPLAY=$X11_DISPLAY --security-opt=label=type:container_runtime_t $IDMAP_ARGS $USER_ARGS localhost/ctbot-eclipse-rpi:$VERSION
$XHOST_CMD -si:localuser:$USER
